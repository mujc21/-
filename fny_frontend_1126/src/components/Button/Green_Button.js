...
export default class GreenButton extends Component {
    ...
    Button_Clicked = () =>{
        if(this.props.Index===null){
            this.props.Green_Button_Clicked()
        }
        else{
            this.props.Green_Button_Clicked(this.props.Index)
        }
    }

    // 注意不要忘记定义render，返回值就是页面要渲染的东西
    render() {
        return <button class="fny_green_button" style={this.props.Green_Button_Style} onClick={this.Button_Clicked}>{this.props.Green_Button_Text}</button>
    }
}
