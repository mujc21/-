...
class MaterialsList extends Component {
  ...
  handleGetOneModelPreviewPicture = (modelID) => {  //返回值为其预览图的路径
    ...
    handleAdd = () => {
        ...
        // const filtered_Array = this.state.All_Materials_List.filter(item => this.state.selectedAllItems.includes(item.id));
        // const modifiedArray = filtered_Array.map((item, index) => ({
        //     ...item,
        //     id: this.state.My_Materials_List.length + index + 1,
        //   }));
        this.setState({ selectedAllItems: [] });
        // this.setState( prevState => ({ 
        //     My_Materials_List: [...prevState.My_Materials_List, ...modifiedArray]
        // }));
    }
            ...
            <List class="materials-post-list" style={this.props.Materials_List_Style}
                ...
                <div style={{ marginLeft: '40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ marginTop: '-5px'}}>
                    <h2>{item.text}</h2>
                    <img src={item.pic} onClick={this.openModal} alt="Sunset" style={{height: '150px', width: '200px'}} />
                  </div>
