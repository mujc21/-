...
def load_kernels(radius, num_kpoints, dimension, fixed, lloyd=False):
    ...
    if not exists(kernel_file):
        ...
        pcd = o3d.io.read_point_cloud(kernel_file)  # 这个文件是怎么给定的kp？？？
    ...
    R = np.eye(dimension)
    theta = np.random.rand() * 2 * np.pi  # 随机一个角度
    if dimension == 2:
        if fixed != 'vertical':
        ...
        if fixed != 'vertical':
            c, s = np.cos(theta), np.sin(theta)
            R = np.array([[c, -s, 0], [s, c, 0], [0, 0, 1]], dtype=np.float32)  # 绕z轴旋转矩阵
        else:  # is vertical
            # 这里会进来吗，什么叫fixed参数是什么意思？？？
