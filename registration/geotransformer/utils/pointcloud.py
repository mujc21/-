...
def get_transform_from_rotation_translation(rotation: np.ndarray, translation: np.ndarray) -> np.ndarray:
    ...
    Returns:
        transform: (4, 4)
    """
    transform = np.eye(4)  # 返回一个对角线为1其余为0的二维矩阵
...
def random_sample_rotation(rotation_factor: float = 1.0) -> np.ndarray:  # 描述了参数类型和返回值类型
    # angle_z, angle_y, angle_x
    euler = np.random.rand(3) * np.pi * 2 / rotation_factor  # 限制了旋转弧度在(0, 2 * pi / rotation_range)之间
    rotation = Rotation.from_euler('zyx', euler).as_matrix()  # 大写是内旋，小写是外旋（绕固定轴），分别沿zyx旋转指定弧度（3,3）
    return rotation
