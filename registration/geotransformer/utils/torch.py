...
def build_dataloader(
    ...
    shuffle=None,
    collate_fn=None,
    pin_memory=False,  # 当计算机的内存充足的时候，可以设置pin_memory=True，电脑性能好才能开
    drop_last=False,
    distributed=False,
):
    if distributed:
    ...
    data_loader = torch.utils.data.DataLoader(
        ...
        drop_last=drop_last,  # 不足一个batch的部分要不要扔掉
    )
...
def initialize(seed=None, cudnn_deterministic=True, autograd_anomaly_detection=False):
    ...
    # torch.backends.cudnn.enabled = True表示可以使用cudnn

    if cudnn_deterministic:  # 表示使用确定算法
        cudnn.benchmark = False  # 禁用
        cudnn.deterministic = True
    else:  # 使用非确定算法，花费时间找最优卷积函数
        cudnn.benchmark = True
        cudnn.deterministic = False
    # 异常检测是否开启，如果开启就会检测反向传播时有没有异常，比如对一个带有梯度信息的tensor进行sin操作就会报错
