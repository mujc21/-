...
export default class App extends Component {
    ...
    handleLogin = async (username, password) => {
      try{
        ...
        await axios.post("http://43.138.68.84:8082/api/login",{
          username,password
        })
        .then(res=>{
          if(res.data === "数据库查询失败"){
    ...
    handleRegister = async (username, password, confirmPassword) => {
      try{
        this.setState({Loading: true})
        await axios.post("http://43.138.68.84:8082/api/register",{
        ...
        .then(res=>{
          ...
          if(res.data === "含有除数字、字母外的其他字符"){
            this.openRegisterModal("含有除数字、字母外的其他字符,请重新输入")
            this.setState({enterRegister:true})
            return
          }
