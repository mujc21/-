...
import OneUser from '../One_User/One_User'
    ...
    render() {
        if(this.props.User_List.length===0){
            return <div class="fny_user_list" style={this.props.Main_Style}> 
                <h1 class="fny_no_user_h">无用户信息</h1>
            </div>
        }
        let temp_Main_Style={}
        ...
        temp_Main_Style['overflowY']='hidden'
        return <div class="fny_user_list" style={this.props.Main_Style}> 
                {
                    //注意下面的写法
                    this.props.User_List.map((one_User,index) => { 
                        ...
                        return <OneUser {...one_User} Main_Style={temp_Main_Style} Picture_Clicked={this.props.Head_Clicked} Ban_User_Clicked={this.props.Ban_User_Clicked} Reset_Password_Clicked={this.props.Reset_Password_Clicked} User_index={index}/>
                    })
                }
            </div>
    }
