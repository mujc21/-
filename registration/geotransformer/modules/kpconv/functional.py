...
def nearest_upsample(x, upsample_indices):
    ...
    x = torch.cat((x, torch.zeros_like(x[:1, :])), 0)  # (n1+1, d)
    # Get features for each pooling location [n2, d]
    x = index_select(x, upsample_indices[:, 0], dim=0)  # (n2, d)
    return x
...
def maxpool(x, neighbor_indices):
    ...
    """
    x = torch.cat((x, torch.zeros_like(x[:1, :])), 0)  # (n1+1, d)
    neighbor_feats = index_select(x, neighbor_indices, dim=0)  # (n2, max_num, d)
    pooled_feats = neighbor_feats.max(1)[0]  # (n2, d)  [0]返回最值，[1]返回索引
    return pooled_feats
