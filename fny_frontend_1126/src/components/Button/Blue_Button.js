...
export default class BlueButton extends Component {
    ...
    Button_Clicked = () =>{
        if(this.props.Index===null){
            this.props.Blue_Button_Clicked()
        }
        else{
            this.props.Blue_Button_Clicked(this.props.Index)
        }
    }

    // 注意不要忘记定义render，返回值就是页面要渲染的东西
    render() {
        return <button class="fny_blue_button" style={this.props.Blue_Button_Style} onClick={this.Button_Clicked}>{this.props.Blue_Button_Text}</button>
    }
}
