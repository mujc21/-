...
class GroupNorm(nn.Module):
    def __init__(self, num_groups, num_channels):
        ...
        self.norm = nn.GroupNorm(self.num_groups, self.num_channels)  # 将channel切分成许多组进行归一化
...
class UnaryBlock(nn.Module):
    def __init__(self, in_channels, out_channels, group_norm, has_relu=True, bias=True, layer_norm=False):
        ...
        if has_relu:
            self.leaky_relu = nn.LeakyReLU(0.1)  # 负轴斜率
        else:
...
class ConvBlock(nn.Module):
    def __init__(
        ...
        if layer_norm:
            ...
            self.norm = GroupNorm(group_norm, out_channels)
        self.leaky_relu = nn.LeakyReLU(negative_slope=negative_slope)  # 激活函数，保留了一部分负轴的信息
...
class ResidualBlock(nn.Module):
    def __init__(
        ...
        mid_channels = out_channels // 4  # 为什么是这个值？？？

        if in_channels != mid_channels:  # step into 2.1.2.1：UnaryBlock，这就是一个MLP加上正则化和激活函数
            self.unary1 = UnaryBlock(in_channels, mid_channels, group_norm, bias=bias, layer_norm=layer_norm)
        else:
            self.unary1 = nn.Identity()  # 恒等变换
    ...
    def forward(self, s_feats, q_points, s_points, neighbor_indices):
        ...
        if self.strided:  # step into 2.1.2.2：maxpool
            shortcut = maxpool(s_feats, neighbor_indices)
        else:
            shortcut = s_feats
        shortcut = self.unary_shortcut(shortcut)

        x = x + shortcut  # 为什么要加一下？？？
        x = self.leaky_relu(x)
