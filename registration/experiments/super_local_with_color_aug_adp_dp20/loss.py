...
class Evaluator(nn.Module):
    ...
    def evaluate_coarse(self, output_dict):
        ref_length_c = output_dict['ref_points_c'].shape[0]  # 最后阶段的ref
        src_length_c = output_dict['src_points_c'].shape[0]  # 最后阶段的src
        gt_node_corr_overlaps = output_dict['gt_node_corr_overlaps']
        gt_node_corr_indices = output_dict['gt_node_corr_indices']
    ...
    @torch.no_grad()
    # step into 6.3 最重要的指标评估
    def evaluate_registration(self, output_dict, data_dict):
        transform = data_dict['transform']
        est_transform = output_dict['estimated_transform']
        src_points = output_dict['src_points']
        # step into 6.3.1 rre和rte
        rre, rte = isotropic_transform_error(transform, est_transform)
        # 这里transform可以直接用逆吗？？？
        realignment_transform = torch.matmul(torch.inverse(transform), est_transform)  # (4, 4)
        realigned_src_points_f = apply_transform(src_points, realignment_transform)


        # rmse = torch.linalg.norm(realigned_src_points_f - src_points, dim=1).mean()  # 这里的rmse是所有点的误差平均值 这种计算rmse的方法需要记一下！！！
        # rmse = torch.sqrt(rmse)
        mse = torch.mean(torch.sum((realigned_src_points_f - src_points) ** 2, dim=1))
        rmse = torch.sqrt(mse)
        recall = torch.lt(rmse, self.acceptance_rmse).float()  # 这句话导致rr只能是0或者1？？？
        # rmse = torch.linalg.norm(realigned_src_points_f - src_points, dim=1)
        # recall = torch.lt(rmse, self.acceptance_rmse).sum()/src_points.shape[0]
        # rmse = rmse.mean()
        # print(rmse)
        # if 0.195 < rmse < 0.2:
        #     src_points = output_dict['src_points']
    ...
    def forward(self, output_dict, data_dict):
        # step into 6.1 算PIR，暂时不看
        c_precision = self.evaluate_coarse(output_dict)
        # step into 6.2 算IR，暂时不看
